...
### **Frontend Architecture: Mandated Framework and Principles**

This guide defines the **mandatory** frontend technology and architectural principles for this project. The AI model must strictly adhere to these rules. There is no decision to be made; the choice has been finalized.

#### **1. Web-First Applications (The Only Supported Platform)**

**Goal:** To build a feature-rich, data-driven, and highly interactive admin dashboard as a Single Page Application (SPA). The primary user experience is exclusively in a web browser.

**Mandated Technology: Vite + React**
*   **Framework:** React 19 with TypeScript.
*   **Build Tool:** Vite.
*   **Runtime Environment:** Node.js (v18.x or later, v20.x recommended).

**Use Case & Implementation Rules:**
This stack is mandated for building the project's core features, which include internal tools, admin dashboards, and complex, data-driven panels. The priority is maximum developer velocity and a highly responsive user interface.
*   All new features must be built as React components using TypeScript (`.tsx` files).
*   The application must be configured and run using Vite. Its best-in-class hot-reloading capabilities are essential to our development workflow.
*   SEO is not a concern for this application type, which is why a pure client-side SPA architecture has been chosen.

**Forbidden Alternatives (Web):**
The following technologies are explicitly **forbidden** for this project. Do not suggest, use, or generate code for them:
*   **Next.js:** Its server-side rendering (SSR) and static site generation (SSG) capabilities are unnecessary for this project's scope and would introduce unwanted complexity.
*   **Angular:** While powerful for large-scale enterprise applications, its opinionated structure is not aligned with our chosen lightweight and flexible stack.

#### **2. Mobile Applications (Out of Scope)**

**Goal:** This project is web-focused. Native mobile application development is **out of scope**.

**Forbidden Technology (Mobile):**
Under no circumstances should **Flutter** or any other mobile-native framework (like React Native) be used, suggested, or implemented. If a requirement seems to imply mobile functionality, you must state that it falls outside the project's current scope.

---
### **Mandatory Action Rubric**

| If the task is to... | Then the required action is... |
| :--- | :--- |
| Build a new UI feature or page | Create React components using TypeScript and Vite. |
| Set up the development environment | Use `npm run dev` (or equivalent) to start the Vite development server. |
| Suggest a full-stack solution | Propose a separate, decoupled backend (e.g., Spring Boot API); **do not use Next.js API routes**. |
| Address a requirement for a mobile app | State clearly that native mobile development is **out of scope**. |
</details>

<details>
<summary>TECH_GUIDE:BACKEND_ARCHITECTURE</summary>
### **Backend Architecture: Mandated Stack and Implementation Principles**

This guide defines the **mandatory** backend technology stack and architectural patterns for this project. The AI model must strictly adhere to these rules for all code generation, refactoring, and analysis tasks. The technology stack is not open for discussion or alternatives.

#### **1. The Mandated Stack: Java & Spring Boot**

The entire backend is to be developed using the Java ecosystem. This choice prioritizes performance, robustness, and maintainability for building enterprise-grade services.
...
**Core Philosophy & Implementation Rules:**
*   **Dependency Injection:** All components **must** be managed by the Spring IoC container and use constructor injection.
*   **Boilerplate Reduction:** Use **Project Lombok** annotations (`@Data`, `@Service`, `@RestController`, etc.) to keep the code clean and concise.
*   **Configuration:** All application properties **must** be managed in the `src/main/resources/application.properties` file.

#### **2. The Mandated Architectural Pattern: Layered Architecture**

All new features and business logic **must** follow a strict three-layer architecture pattern: Controller, Service, and Repository.

**A. Controller Layer (`@RestController`)**
*   **Responsibility:** To handle incoming HTTP requests, parse request bodies, and delegate work to the Service layer.
*   **Implementation:** Controllers **must not** contain any business logic. They are responsible for API contract definition and validation.

**B. Service Layer (`@Service`)**
*   **Responsibility:** To contain all business logic, orchestrate operations, and enforce business rules.
*   **Implementation:** Services are the only place where business logic should reside.

**C. Repository Layer (`@Repository`)**
*   **Responsibility:** To handle all data persistence and retrieval operations with the PostgreSQL database.
*   **Implementation:** This layer **must** be implemented using **Spring Data JPA** by creating interfaces that extend `JpaRepository`.

#### **3. Integration with Supabase as Backend-as-a-Service (BaaS)**

While Spring Boot provides the application framework, **Supabase is the managed service provider for Authentication and the Database**. This division of responsibility is critical and must be respected.

**A. Authentication (Supabase GoTrue)**
*   **Source of Truth:** Supabase GoTrue is the **sole authority for user identity and authentication**. User sign-up, sign-in, and password management are handled by Supabase, not by our Spring Boot backend.
*   **Frontend's Role:** The React frontend interacts **directly** with the Supabase client library (`supabase-js`) to handle all user authentication flows (login, signup, etc.). Upon successful login, the frontend receives a **JWT (JSON Web Token)** from Supabase.
*   **Backend's Role (CRITICAL):** The Spring Boot backend's role is **exclusively to validate the Supabase-issued JWT** on every protected API request.
    *   **Implementation:** Configure **Spring Security** as a **Resource Server** for JWT validation.
    *   The security configuration **must** be set up to verify the token's signature against Supabase's public key (JWK Set URI) or the project's JWT secret.
    *   **DO NOT** create endpoints like `/api/auth/login` or `/api/auth/register` in the Spring Boot application. This is a violation of the architecture.

**B. Database and Table Management (Supabase PostgreSQL)**
*   **Source of Truth:** Supabase provides the managed PostgreSQL database instance. The database schema (tables, columns, relations, constraints, and RLS policies) is managed **directly within the Supabase dashboard** or via Supabase migrations.
*   **Backend's Role:** Our Spring Boot application interacts with this database using **Spring Data JPA** as its data access layer.
    *   **Implementation:** The JDBC connection details in `application.properties` (`spring.datasource.url`, `username`, `password`) **must** be configured with the credentials from the Supabase project's database settings.
    *   **CRITICAL RULE:** The Spring Boot property `spring.jpa.hibernate.ddl-auto` **must be set to `validate` or `none`** in all environments. **Never use `create` or `update`**, as this would cause our application to attempt to alter the Supabase-managed schema, leading to conflicts and potential data loss. Supabase is the owner of the schema.

#### **4. Forbidden Alternatives**

To ensure a consistent and maintainable codebase, the following technologies are explicitly **forbidden** for this project. Do not suggest, use, or generate code for them:
*   **Node.js (NestJS, Express):** The backend will be written exclusively in Java.
*   **Python (FastAPI, Django):** Not to be used for any backend service development.
*   **Go (Golang):** Not to be used.

---

### **Mandatory Action Rubric**

The following rubric is not a guide for making decisions; it is a set of **non-negotiable rules**. When tasked with a specific development goal, you **must** use the specified technology and architectural pattern without deviation.

| If the task is to... | Then the required action is... |
| :--- | :--- |
| **Build a new UI feature or page** | Create **React components** using **TypeScript** and the **Vite** build tool. |
| **Create a new API endpoint** | Implement it in a Spring Boot `@RestController`, which calls a `@Service` method. |
| **Add or modify business logic** | Implement the logic exclusively within a Spring Boot `@Service` class. |
| **Access or modify data in the database** | Use a **Spring Data JPA** `@Repository` interface that connects to the Supabase PostgreSQL database. |
| **Handle user login/signup** | State that this is a **frontend responsibility** that uses the Supabase client library directly. **Do not create backend code for this.** |
| **Secure an endpoint** | Use **Spring Security** to validate the **Supabase-issued JWT** provided by the frontend. |
| **Change the database schema (tables, columns, etc.)** | State that this action must be performed **in the Supabase dashboard** or via a Supabase migration script. **Do not use `ddl-auto`**. |
| **Add a new library/dependency** | Add it to the `pom.xml` (Backend) or `package.json` (Frontend) file. |
</details>
...
### **Backend Data Access: Mandated ORM and Persistence Layer**

*   **Problem:** Writing raw SQL queries is error-prone, hard to maintain, and not type-safe. This approach couples business logic directly to the database schema and increases the risk of SQL injection vulnerabilities.

*   **Solution:** An Object-Relational Mapper (ORM) maps your database tables to Java objects called **Entities**. This allows developers to interact with the database using object-oriented code, which is type-safe, more maintainable, and database-agnostic.

*   **Mandated Technology: Spring Data JPA & Hibernate**
    For this project, the data access layer is exclusively managed by the Spring Data ecosystem.

    *   **High-Level Abstraction: Spring Data JPA**
        This is the framework used to define repository interfaces. It provides a powerful and consistent, Spring-based model for data access, significantly reducing the amount of boilerplate code required.

    *   **Underlying ORM Implementation: Hibernate**
        Spring Data JPA uses Hibernate as its default implementation. Hibernate is responsible for the actual object-relational mapping, translating method calls from our repository interfaces into efficient SQL queries.

    *   **Implementation Pattern:**
        1.  **Entities (`@Entity`):** All database tables managed in Supabase **must** be mapped to a corresponding Java class annotated with `@Entity`. These classes define the object model of our data and reside in the `com.cambiaso.ioc.entity` package.
        2.  **Repositories (`@Repository`):** All data access operations (Create, Read, Update, Delete) **must** be performed through repository interfaces that extend `JpaRepository`. This approach provides standard persistence methods out-of-the-box and ensures a clean, testable data access layer. These interfaces reside in the `com.cambiaso.ioc.repository` package.

*   **Non-Negotiable Rule:**
    All database interaction **must** be implemented using **Spring Data JPA repositories**. Direct use of JDBC or raw SQL queries is forbidden unless absolutely necessary for a highly complex, performance-critical query that has been explicitly approved. The use of other ORMs such as **Prisma** or **SQLAlchemy** is strictly forbidden.
</details>
